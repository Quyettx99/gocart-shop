...
export default function StoreAddProduct() {
  ...
  // Xử lý thay đổi giá tiền - loại bỏ dấu chấm và ký tự không phải số
  const handlePriceChange = (e) => {
    const name = e.target.name;
    // Loại bỏ tất cả ký tự không phải số
    const value = e.target.value.replace(/[^\d]/g, "");
    const numValue = value === "" ? 0 : parseFloat(value) || 0;
    setProductInfo({ ...productInfo, [name]: numValue });
  };

  const onChangeHandler = (e) => {
    // Nếu là trường giá tiền, sử dụng handlePriceChange
  ...
  return (
    <form
      onSubmit={(e) =>
        toast.promise(onSubmitHandler(e), { loading: "Đang thêm sản phẩm..." })
      }
      className="text-slate-500 mb-28"
    >
      <h1 className="text-2xl">
        Thêm sản phẩm <span className="text-slate-800 font-medium">mới</span>
      </h1>
          ...
          <div className="relative">
            <input
              ...
              value={
                productInfo.mrp === 0
                  ? ""
                  : productInfo.mrp.toLocaleString("vi-VN")
              }
              placeholder="0"
              className="w-full max-w-45 p-2 px-4 pr-20 outline-none border border-slate-200 rounded resize-none"
              required
            />
            {productInfo.mrp > 0 && (
